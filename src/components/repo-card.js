window.addEventListener('DOMContentLoaded', async function() {
  ...
  document.querySelectorAll('.repo-card').forEach(async function(el) {
    ...
    <div style="font-family: -apple-system,BlinkMacSystemFont,Segoe UI,Helvetica,Arial,sans-serif,Apple Color Emoji,Segoe UI Emoji; border: 1px solid ${theme.borderColor}; border-radius: 6px; background: ${theme.background}; padding: 16px; font-size: 14px; line-height: 1.5; color: #24292e;">
      ...
      <div style="display: ${data.fork ? 'block' : 'none'}; font-size: 12px; dark:text-biancoperla;">Forked from <a style="color: inherit; text-decoration: none;" href="${data.fork ? data.source.html_url : ''}">${data.fork ? data.source.full_name : ''}</a></div>
      <div style="font-size: 12px; margin-bottom: 16px; margin-top: 8px; dark:text-biancoperla;">${data.description}</div>
      <div style="font-size: 12px; dark:text-biancoperla; display: flex;">
